...
public class EsqueletoArmado extends Enemy {
    private Random random = new Random();

    public EsqueletoArmado(int x, int y, int health, int damage) {
    ...
    public void moveTowardsPlayer(PlayerCharacter player) {
        ...
        for (Trap t : logic.getTraps()) {
            if (t.getX() == newX && t.getY() == newY && t.isActive() && t.getType().equals("Pitfall")) {
                health = 0;
                logic.addGameMessage("Esqueleto Armado cae en una trampa y muere.");
                return;
            }
        }
    ...
    public void attack(PlayerCharacter player) {
        if (Math.abs(player.getX() - x) <= range && Math.abs(player.getY() - y) <= range) {
            ...
            GameLogic.getInstance().addGameMessage("Esqueleto Armado ataca a " + player.getName() + " (-" + damage + " HP).");
        }
    }
}
